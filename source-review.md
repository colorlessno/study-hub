...
| 7 | 対応済み | 公開用CIを`portfolio-validation.yml`へ集約し、StudyHubのビルド・全テスト後に13個の再利用ワークフローを`needs`で一件ずつ呼ぶ。matrixも`max-parallel: 1`へ制限し、全ジョブへ制限時間、全ワークフローへ`contents: read`だけの権限を設定した。外部Actionは公式タグの参照先コミットへ固定し、checkout後の認証情報保持を無効化した。全14ワークフローの構成を検証スクリプトで固定した。 |
