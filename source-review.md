...
| 7 | 対応済み | 公開用CIを`portfolio-validation.yml`へ集約し、StudyHubのビルド・全テスト後に13個の再利用ワークフローを`needs`で一件ずつ呼ぶ。matrixも`max-parallel: 1`へ制限し、全ジョブへ制限時間、全ワークフローへ`contents: read`だけの権限を設定した。Linux runnerはUbuntu 24.04、Windows runnerはWindows Server 2025へ固定し、`latest`切替によるOS世代の変化を避けた。外部Actionは公式タグの参照先コミットへ固定し、checkout後の認証情報保持を無効化した。秘密情報を使う検証、権限境界を変えるトリガー、未信頼イベント値のシェル展開も禁止した。Node.js依存はlockfileを使う`npm ci`へ統一し、CI、教材内workflow、DockerコンテナのNode.js実行系は保守対象の22へ統一した。StudyAIのPython依存はバックエンドの`pyproject.toml`だけから導入して同ファイルをpipキャッシュキーにした。失敗時のPlaywright成果物は隠しファイルを除外し、7日で削除する。全14ワークフローの構成とNode.js 20実行設定の不在を検証スクリプトで固定した。 |
