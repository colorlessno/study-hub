...
**取り込み時の前処理**
- 音声・動画ファイル：faster-whisperで区間単位に文字起こしした後、ローカルの`pyannote/speaker-diarization-community-1`を順番に実行する。各文字起こし区間へ時刻の重なりが最大の匿名話者ラベルを割り当て、重なりがない区間は`unknown`とする。匿名ラベルを顧客・担当者へ推測で割り当てない。モデル未配置・読込み失敗・推論失敗は取込失敗として明示し、全区間`unknown`への自動切替は行わない
...
### POST /data/upload
データをアップロードし、サーバープロセス共通の受付順に、正規化、匿名化、分析、グルーピング、DB保存を一件ずつ順番に実行する。応答の`status`はDBへ保存した最終状態と一致させ、途中で失敗した場合は`failed`を返す。
...
### system14_conversationsテーブル
```sql
CREATE TABLE system14_conversations (
    id            SERIAL PRIMARY KEY,
    job_id        VARCHAR(50) REFERENCES system14_data_jobs(id),
    data_type     VARCHAR(20),
    source        VARCHAR(50),
    transcript    TEXT,
    metadata      JSONB,
...
### system14_utterancesテーブル（発言単位）
```sql
CREATE TABLE system14_utterances (
    id              SERIAL PRIMARY KEY,
    conversation_id INTEGER REFERENCES system14_conversations(id),
...
### system14_insight_groupsテーブル（意味グルーピング）
```sql
...
- データは1ファイルずつ受付要求内で順番に処理し、完了・失敗状態をDBに保存する
...
| 話者分離 | pyannote speaker-diarization-community-1（ローカル、CPU） |
...
| 工程4：実装 | faster-whisper区間文字起こし・pyannote話者分離・発話時刻保存・ルール分析・LLM／LangGraph分析・FAQ／発話／営業スコア／CRM履歴のRAG・Webhook・SMTP・ダッシュボード配信・DB配信ログ |
